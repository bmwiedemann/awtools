   ...
   if(defined($pid) && $pid==0) { # child process
      {
         ...
         foreach my $n (qw(42713 49387 50080 114704 276532 281335  150685 281285)) {
            push(@site, "Player/Profile.php/?id=$n");
         }
			my $host=$awstandard::bmwproxyaddr;
         my $lasturi="http://$host/0/News/";
         for my $n(1..$times) {
            if($bg) { sleep 60; }
            my $now=time();
            my %lus=%{$us{$sessionid}};
...
#            my @lus=%lus; print "@lus\n";
               ...
               print F "$n $lus{nclick} $$ $now $uri ",length($content),"\n";
               close F;
					open(F, ">/tmp/awstaydebug.html"); print F $content;close F;
            }
         }
         untie %us;
      }
...
$r->print( $q->start_html(-title=>"AW stay"),$m,$q->br,
#$q->start_form(-name=>"form", -method=>"get"),
"<form>",
$q->textfield(-name=>'time', -class=>'text')," minutes",$q->br,

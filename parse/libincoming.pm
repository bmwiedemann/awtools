...
sub parse_incoming($)
{
	...
	# GE27 beta format
	$html=~m{<td class="incoming">\s*($awstandard::awdatere)<br />\s*.*index.php\?destroyer=(\d+)&amp;cruiser=(\d+)&amp;battleship=(\d+).*\b(\d+) TR(?:</span>)?\s*(?:(\d+) CS\s*)?-.*<a href="\.\./Map/Detail\.php\?nr=(\d+)">ID \d+ - ([^#]+) #(\d+)</a><br />\s*<a href="\.\./Player/Profile\.php\?id=(\d+)">(?:\[\w+\])? *([^<]+)</a>}s or return;
